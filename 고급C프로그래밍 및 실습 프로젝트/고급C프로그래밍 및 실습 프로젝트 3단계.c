...
int RegFromFile(struct tel tel_list[], int count, int max_num){
	char tmp_name[100], tmp_tel_no[100], tmp_birth[100];
	int i, smaller_than;
	FILE *fp;
	fp = fopen("PHONE_BOOK.txt", "r");

	fscanf(fp, "%s %s %s", tmp_name, tmp_tel_no, tmp_birth);
	while (!feof(fp)){
		//fscanf(fp, "%s %s %s", tmp_name, tmp_tel_no, tmp_birth);
...
void Exit(struct tel tel_list[], int count){
	int i;
	FILE *fp;
...
int main(){
	...
	while(1){
		...
		if(Select == 1){
			...
			count = RegFromFile(tel_list, count, max_num);
		}
		else {
			Exit(tel_list, count);
			break;
